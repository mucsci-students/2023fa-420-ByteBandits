...
public class helpers {
    ...
    public static String dynamicHints(String baseWord, char reqLetter) throws FileNotFoundException {
        ...
        matrixStr.append(
                "<b style=\" font-family: 'Trebuchet MS', sans-serif; font-size: 20px;\">Wordy Wasps Grid</b><br><br><br>");
        matrixStr.append(
                "<span style=\" font-family: Garamond, serif; font-size: 16px;\">Center letter is in <b>bold</b>.</span><br><br>");
        matrixStr.append(
                "<span style=\" font-family: Garamond, serif; font-size: 18px;\">" + wordWithBold + "</span><br><br>");

        matrixStr.append("<span style=\" font-family: Garamond, serif; font-size: 16px;\">WORDS: " + totalWords
                + ", POINTS: " + possiblePoints + ", PANGRAMS: " + totalPangrams[0] + " (" + totalPangrams[1]
                + " Perfect)" + bingo + "</span><br><br><br><br>");

        matrixStr.append("<table style=\"border-collapse: collapse;\">"); // Start the table with collapsed borders

        matrixStr.append("<tr>");
        matrixStr.append(
                "<td style=\"font-family: Garamond, serif; font-size: 19px; border: none; padding: 10px;\"></td>"); // Empty
                                                                                                                    // first
                                                                                                                    // element
        for (int j = 4; j <= longestWordLength; j++) {
            matrixStr.append(
                    "<td style=\"font-family: Garamond, serif; font-size: 19px; border: none; padding: 10px;\">");
        ...
        for (int i = 0; i < hintsMatrix.length; i++) {
            ...
            if (index < 7) {
                ...
                matrixStr.append(
                        "<b style=\" font-family: Garamond, serif; font-size: 16px;\">&#8721 : &nbsp;&nbsp;</b>");
            }
            ...
            for (int j = 0; j < hintsMatrix[i].length; j++) {
                ...
                if (i == hintsMatrix.length - 1 || j == hintsMatrix[i].length - 1) {
                    matrixStr.append(
                            "<td style=\"font-family: Garamond, serif; font-size: 19px; border: none; padding: 10px;\"><b>")
                            .append(cellData).append("</b></td>"); // Make the last row and column bold
                } else if (cellData.startsWith("<b>")) {
                    matrixStr.append(
                            "<td style=\"font-family: Garamond, serif; font-size: 19px; border: none; padding: 10px;\">")
                            .append(cellData).append("</td>"); // Keep the existing bold cells
                } else {
                    matrixStr.append(
                            "<td style=\"font-family: Garamond, serif; font-size: 19px; border: none; padding: 10px;\">")
                            .append(cellData).append("</td>"); // For other cells
                }
            }
            matrixStr.append("</tr>");
        }
    ...
    public static String[][] getDynamicMatrix(List<String> acceptedWordList, int longestWordLength, String baseWord,
            char reqLetter) {
        ...
        for (int i = 0; i < baseWord.length(); i++) {
            ...
            for (Map.Entry<String, Integer> entry : pairCountMap.entrySet()) {
                if (entry.getKey().charAt(0) == baseChar) {
                    html.append("<span style=\" font-family: Garamond, serif; font-size: 18px;\">"
                            + entry.getKey().toUpperCase()).append("-").append(entry.getValue())
                            .append("&nbsp;&nbsp;</span>");
                }
            }
    ...
    public static void dynamicHintsCLI(String baseWord, char reqLetter) throws FileNotFoundException {
        ...
        System.out.printf(
                "WORDS: " + yellowColor + "%d" + resetColor + ", POINTS: " + yellowColor + "%d" + resetColor
                        + ", PANGRAMS: " + yellowColor + "%d" + resetColor + " (%d Perfect)%s\n\n",
    ...
    /*
     * getDynamicMatrixCLI
     * param: List<String> acceptedWordList, int longestWordLength, String baseWord,
     * char reqLetter
     * returns: String[][]
     * This function creates the matrix of numbers representing
     * the amount of words in the accepted word list based on the
    ...
    public static String[][] getDynamicMatrixCLI(List<String> acceptedWordList, int longestWordLength, String baseWord,
            char reqLetter) {
        String yellowColor = "\u001B[33m";
    ...
    public static void getDynamicTwoLetterListCLI(List<String> acceptedWords, String baseWord) {
        ...
        for (int i = 0; i < baseWord.length(); i++) {
            ...
            for (Map.Entry<String, Integer> entry : pairCountMap.entrySet()) {
                if (entry.getKey().charAt(0) == baseChar) {
                    System.out.printf("%s-" + yellowColor + "%d\t" + resetColor, entry.getKey().toUpperCase(),
                            entry.getValue());
                }
            }
            prevBaseChar = baseChar;
     ...
     * getTotalPangrams
     * param: String baseWord, List<String> acceptedWords
     * returns: int[] containing int[0]: totalPangrams and int[1]: totalPerfect
     * pangrams.
     * This function simply looks at the acceptedWordList of all
     ...
     * param: String[][] matrix
     * returns: boolean
     * This function receives the matrix of word counts. It then will check every
     * row
     * of the last column exept for the first and last row (as they are headings and
     * sums).
     * It checks those numbers and if any of them is <1 then it is not a BINGO
     * puzzle and returns false,
     ...
     * param: String currentRank, int playerPoints, List<String> possibleWords,
     * String baseword
     * returns: N/A
    ...
    public static void calculateRankDifference(String currentRank, int playerPoints, List<String> possibleWords,
            String baseWord) {
        nextRank = "";
        pointsRequired = 0;

        int currentRankIndex = Arrays.asList("Beginner", "Good Start", "Moving Up", "Good", "Solid", "Nice", "Great",
                "Amazing", "Genius", "Queen Bee").indexOf(currentRank);
        ...
        if (currentRankIndex >= 0) {
            ...
            System.out.println("TOTAL POINTS NEEDED FOR NEXT RANK " + "\u001B[33m" + nextRank + "\u001B[0m" + ": "
                    + "\u001B[33m" + pointsRequired + "\u001B[0m");
